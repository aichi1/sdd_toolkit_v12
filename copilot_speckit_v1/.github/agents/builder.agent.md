---
name: sdd-builder
description: SKILL.md の手順と docs/ の要件に従って成果物を生成し outputs/phase-{N}/ に配置する。自己評価はしない（検証は sdd-validator の担当）。/run-phase の生成フェーズで使う。
...
> Copilot ではフロントマターの `tools:` が実際の権限。以下はその意図の説明。

### フルアクセス
- `execute`（シェル実行） - コマンド実行（制限なし）
- `edit`（ファイル作成） - ファイル作成
- `edit`（ファイル編集） - ファイル編集
- `read`（ファイル・ディレクトリ閲覧） - ファイル・ディレクトリ閲覧

### 使用例
```bash
# データ処理
shell("python analyze.py data.csv")

# 成果物作成
write_file("./outputs/phase-01/report.md", content)

# 既存ファイル修正
edit_file("./outputs/phase-01/draft.md", old_text, new_text)

# 要件確認
read_file("./docs/background.md")
```
...
```python
# 良い例: 段階的に保存
write_file("outputs/phase-02/outline.md", outline)
write_file("outputs/phase-02/section1.md", section1)
write_file("outputs/phase-02/section2.md", section2)
# → 途中で問題が起きても部分的に残る

# 悪い例: 最後に一気に保存
all_content = generate_everything()
write_file("outputs/phase-02/report.md", all_content)
# → 途中で失敗すると全ロス
```
...
- **明示指定（CLI 非対話）**: `copilot --agent sdd-builder -p "..."`。
