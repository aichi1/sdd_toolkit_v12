---
name: update-docs
description: docs/ 配下の仕様ドキュメントを対話的に充実させる。充実度診断、未解決マーカー（TODO / 要確認）の洗い出し、skills/ との整合チェック、優先度付きの改善提案を行う。フェーズ実行前の仕様固めに使う。
...
3. 各不足箇所について、以下のいずれかを実行:
   a. ユーザに質問して情報を取得する
   b. Copilot自身の知識で補完できる場合は提案し、ユーザの承認を得る
...
2. 以下をチェック:
   ...
   - AGENTS.md との整合性（プロジェクト概要と各docsの内容が一致しているか）
3. 問題を報告し、修正を提案する
...
- Copilot自身の知識で補完する場合は、必ず「Copilot の知識に基づく提案」と明示する
