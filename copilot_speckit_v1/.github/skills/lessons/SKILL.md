---
name: lessons
description: 過去のレトロスペクティブから蓄積された学び（Lessons）を検索・表示し、カテゴリ別・横断的なパターンと推奨アクションを提示する。「過去の教訓を見たい」「似たプロジェクトの学びは？」というときに使う。
argument-hint: [カテゴリ名 | キーワード] [--priority high|medium|low|all]
