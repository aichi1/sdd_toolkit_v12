---
name: run-phase
description: Builder / Validator パターンで SDD のフェーズを実行する。Builder が outputs/phase-{N}/ に成果物を生成し、Validator が docs/ と SKILL.md の要件に照らして検証し、NEEDS_REVISION なら最大2回まで自動修正する。フェーズを進めたい・成果物を作りたいときに使う。
...
- コンテキスト：`AGENTS.md`、`docs/`、`skills/phase-{N}/SKILL.md`
- （あれば）前フェーズの出力
...
## GitHub Copilot でのエージェント委譲

Builder と Validator は `.github/agents/*.agent.md` に定義されたカスタムエージェントである。
本 spec-kit では **必ず別コンテキストへ委譲**すること（同一コンテキストで自作自演すると
Builder が自己検証してしまい、Builder/Validator 分離の意味が失われる）。

### 委譲のしかた

| 実行環境 | Builder の起動 | Validator の起動 |
|---|---|---|
| Copilot CLI（対話） | `/agent` で `sdd-builder` を選択 | `/agent` で `sdd-validator` を選択 |
| Copilot CLI（非対話） | `copilot --agent sdd-builder -p "phase {N} の成果物を生成"` | `copilot --agent sdd-validator -p "phase {N} を検証"` |
| VS Code | チャットのエージェント選択、またはプロンプトファイルの `agent:` | 同左 |
| 自動委譲 | 「フェーズ{N}の成果物を作って」と依頼すると description に合致して自動委譲される | 「フェーズ{N}を検証して」で同様 |

### 委譲時に必ず渡すコンテキスト
サブエージェントは隔離コンテキストで動くため、**会話履歴は引き継がれない**。
起動プロンプトに以下のファイルパスを明示すること。

- Builder へ: `skills/phase-{N}/SKILL.md`、`docs/` 一式、`AGENTS.md`、前フェーズの `outputs/phase-{N-1}/`
- Validator へ: `skills/phase-{N}/SKILL.md`（Quality Criteria）、`docs/` 一式、
  `outputs/phase-{N}/`、`outputs/phase-{N}/.metadata.json`

### 引き継ぎはファイル契約で行う
- Builder → Validator: `outputs/phase-{N}/.metadata.json`
- Validator → Builder: `outputs/phase-{N}/.validation/report.md`
- 修正後の Builder → Validator: `.metadata.json` の `revision_history` を追記

## Builder / Validator エージェント定義

**Builder エージェント**（`.github/agents/builder.agent.md`）:
```yaml
name: SDD Builder
role: Generate deliverables according to SKILL.md
tools: [read, search, edit, execute]   # Copilot のツール別名
constraints:
...
**Validator エージェント**（`.github/agents/validator.agent.md`）:
```yaml
name: SDD Validator
role: Verify deliverables against requirements
tools: [read, search]                  # 読み取り専用（edit / execute を与えない）
constraints:
