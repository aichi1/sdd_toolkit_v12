---
name: init-task
description: SDD プロジェクトを初期化する。タスク分類、docs/（What）の生成、フェーズ別 skills/（How）の作成、専門家エージェントの召喚、AGENTS.md と metadata.json の作成までを行う。新しい仕様駆動プロジェクトを立ち上げたいとき、「仕様から作りたい」「プロジェクトを初期化したい」と言われたときに使う。
argument-hint: 作りたいものの説明（例: 社内向けの技術調査レポートを作りたい）
...
- `.github/agents/*.agent.md` に専門家エージェント
- `./AGENTS.md`（プロジェクト仕様）
...
`templates/team-roster.json` を読み、task_type に対応する specialists を `.github/agents/` に生成する。

- 生成元テンプレ：`templates/agents/<specialist_key>.md`
- 生成先：`.github/agents/<specialist_key>.agent.md`
  - **拡張子は必ず `.agent.md`**（Copilot はこの拡張子でカスタムエージェントを認識する）
  - Copilot のエージェント探索はフラットな `.github/agents/` を対象とするため、
    サブディレクトリ（`generated/`）は作らない
- frontmatter は `name` / `description` / `tools` のみを使う（`model:` は Copilot CLI では無視される）
- 生成時に、`description` に「今回タスクの要点」を1行追記して良い
  （description は自動委譲のトリガーになるため、タスク固有語を入れると精度が上がる）
- 既に同名がある場合は上書きせず、差分提案として提示
- 生成したファイル名を `metadata.json` の `generated_agents` 配列に記録する
  （`/finalize` のアーカイブ・クリーンアップ対象になる）
