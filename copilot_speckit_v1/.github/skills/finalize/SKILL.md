---
name: finalize
description: SDD プロジェクトの完了処理を行う。成果物のパッケージ化、~/.sdd-knowledge/docs-archive/ へのアーカイブ、再利用スターターの抽出、finalization-report.md の生成、metadata.json の finalized 化。「プロジェクトを完了したい」「成果物をまとめたい」ときに使う。
argument-hint: --skip-archive | --skip-starter | --force
...
- AGENTS.md と metadata.json が最新状態になっている
...
if [ -d .github/agents/generated/ ]; then
  mkdir -p $ARCHIVE_DIR/agents/
  cp .github/agents/generated/*.md $ARCHIVE_DIR/agents/
fi
if [ -d .github/agents/ ]; then
  mkdir -p $ARCHIVE_DIR/agents/
  cp .github/agents/*.md $ARCHIVE_DIR/agents/ 2>/dev/null || true
fi

# プロジェクトメタデータ
cp ./AGENTS.md $ARCHIVE_DIR/meta/
...
```json
{
  ...
  "description": "{one-line summary from AGENTS.md}"
}
```
