---
name: retrospective
description: 完了したプロジェクトの構造化された振り返りを行い、再現可能な学びを機械可読な JSON として ~/.sdd-knowledge/retrospectives/ に蓄積する。retrospective.md も生成する。「振り返りをしたい」「反省会」「KPT」と言われたときに使う。
argument-hint: 注目したい領域（省略可）
